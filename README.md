...
> **Assessment submission (MongoDB JDBC):** this repository is
> [`SOWJANYAKAGITHA/helical-insight-mongodb`](https://github.com/SOWJANYAKAGITHA/helical-insight-mongodb)
> — a Helical Insight checkout with a MongoDB JDBC adapter, UI Test→Save fix, metadata listing, and Views preview support.
> Clone URL: `https://github.com/SOWJANYAKAGITHA/helical-insight-mongodb.git`
> Working startup: `bash scripts/setup-dev.sh` then `docker compose -f docker-compose.dev.yml up --build`
> → http://localhost:8080/hi-ee/ (`hiadmin` / `hiadmin`). Details: [docs/mongodb-setup.md](docs/mongodb-setup.md).
...
MongoDB connectivity (Helical JDBC adapter): see [docs/mongodb-setup.md](docs/mongodb-setup.md).
...
```bash
git clone https://github.com/SOWJANYAKAGITHA/helical-insight-mongodb.git
cd helical-insight-mongodb

# One-time: links, env files (safe before the first Maven build)
bash scripts/setup-dev.sh

# Build React (Test→Save fix) + backend WAR, with Mongo + Postgres
docker compose -f docker-compose.dev.yml up --build
```

Open **http://localhost:8080/hi-ee/** — login: `hiadmin` / `hiadmin`

Mongo hostname on this Compose network: **`mongodb`**. Full Mongo guide: [docs/mongodb-setup.md](docs/mongodb-setup.md).

Upstream Helical Insight (unrelated to this submission fork):

```bash
git clone https://github.com/helicalinsight/helicalinsight.git
```

Optional package layout under `docker/` (needs a prior WAR; prefer `docker-compose.dev.yml` for reviewers):
...
Build the **React client + backend from source** (recommended for this MongoDB submission):
...
`Dockerfile.dev` rebuilds `client/` (includes the Test→Save fix) and packages it into the WAR before Tomcat starts.
